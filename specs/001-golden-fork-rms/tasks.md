...
- [x] T029a [US2] **Resolved: status flag added.** `payment_method` gained `status ENUM('Active','Inactive') NOT NULL DEFAULT 'Active'` (schema.sql + idempotent `db/migrations/001_payment_method_status.sql`, applied to the live DB). `PaymentMethodDAO` gained `findActive`/`countActive`/`setStatus`; `SystemConfigService.setPaymentMethodActive` activates/deactivates and refuses to deactivate the last active method (FR-15); the POS lists active methods only; the system-config screen shows a status column and an activate/deactivate button; `data-model.md` §2.10 updated. Verified end-to-end: toggle, keep-≥1-active refusal, and restore.
