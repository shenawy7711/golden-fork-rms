...
- [ ] T049a [US1] **Decision**: reconcile over-cap discount handling. TDD §8.3 ("subtotal 20.00, fixed 25.00 → discount capped at 20.00") and the constitution-gated T043 both require **capping**; `contracts/pos-billing.md` says "Over-cap value → `ValidationException`" and FRD Appendix A says "Reject; 'Discount cannot exceed subtotal'". Implemented as capping, since that is what the gate asserts and matches BR-17's "result never negative". If rejection is wanted instead, T043's expectation and TDD §8.3 must change too — decide which artifact is authoritative and align the other.
